...
class biLSTM_partialGlobal(tf.keras.Model):
...
class biLSTM_tuner_partialGlobal(kt.HyperModel):
    def build(self, hp):
        hp_units = hp.Int('units', min_value = 768, max_value = 768, step = 128)
        model = biLSTM_partialGlobal(hp_units)
...
def train_tract_biRNN_partialGlobal(dirs, pairList_train, pairList_test, featureList, target, lag, tuneTrail, maxEpoch, metaDataDir, randomSeed, dayOfWeekJan1):
    ...
    pairList_train = [i for i in pairList_train if i[0].split('-')[0] in protoListSelected]
    pairList_test = [i for i in pairList_test if i[0].split('-')[0] in protoListSelected]
    protoList = [i for i in protoList if i.split('-')[0] in protoListSelected]

    protoListSelected = ['SingleFamily-2004',
                         # 'MultiFamily-2004',
                         # 'SingleFamily-2013'
                         ]
    pairList_train = [i for i in pairList_train if i[0] in protoListSelected]
    pairList_test = [i for i in pairList_test if i[0] in protoListSelected]
    protoList = [i for i in protoList if i in protoListSelected]
    ...
    print()
    print('---------- Merged data has been encoded ----------')
    ...
    for prototypeSelect in protoList:
        ...
        data_3 = data_encoded.loc[(data['prototype'] == prototypeSelect).tolist()]
        protoClimate = [str(item[1]) for item in pairList_train if item[0] == prototypeSelect]
    ...
    for prototypeSelect in protoList:
        ...
        data_2 = data_encoded.loc[(data['prototype'] == prototypeSelect).tolist()]

        # for saving memory
        data_encoded = data_encoded.loc[(data['prototype'] != prototypeSelect).tolist()]
    ...
    tuner = kt.BayesianOptimization(
        biLSTM_tuner_partialGlobal(),
        objective = 'val_loss',
        max_trials = tuneTrail,
        overwrite = True,
        directory = './tuner',
        project_name = 'biLSTM_partialGlobal ' + str(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    )
